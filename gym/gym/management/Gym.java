...
public class Gym  {
    ...
    public void setSecretary(Person person, int salary) {
        if (this.secretary != null) {
            allSecretaries.add(this.secretary); // שמירת המזכירה הקודמת
        }

        // בדיקה אם person הוא לקוח קיים
        Client c = clientManagement.getClientFromList(person);
        if (c != null) {
            // יצירת Secretary עם הנתונים של הלקוח
            this.secretary = new Secretary(c, salary);
        } else {
            // יצירת Secretary עם הנתונים של Person רגיל
